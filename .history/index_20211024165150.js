...
app.post("/signup",(req,res)=>{
    ...
    if(user){
        ...
        connection.query("insert into users (username,password,fullname) values ()",[],(err,result)=>{
...
app.post("/login", (request, response) => {
    ...
    // });
    console.log(`SELECT * FROM users WHERE users.username='${user.username}'`);
...
app.get("/userMsgs", checkSession, (req, res) => {
   ...
   where 
   messages.to_user_id=${req.session.user.id} 
   or 
   messages.to_user_id is null
   or 
   messages.from_user_id=${req.session.user.id}
   order by
    messages.created_at desc`, (err, result) => {
